...
class Bug42378Test extends Sugar_PHPUnit_Framework_TestCase
{
    ...
    public function testSaveRangeDateFields()
    {
        ...
        $this->assertEquals(
            $formatted_data['range_date_modified_advanced'],
            '2011-03-01',
            "Assert that value is in db format ('2011-03-01')"
        );
        $this->assertEquals(
            $formatted_data['start_range_date_entered_advanced'],
            '2011-03-01',
            "Assert that value is in db format ('2011-03-01')"
        );
        $this->assertEquals(
            $formatted_data['end_range_date_entered_advanced'],
            '2011-03-05',
            "Assert that value is in db format ('2011-03-05')"
        );

        //Last check to see that the macro value is okay and preserved
        $this->assertEquals(
            $formatted_data['range_date_closed_advanced'],
            '[last_7_days]',
            "Assert that the macro date value [last_7_days] was preserved"
        );
        $this->assertEquals(
            $formatted_data['date_closed_advanced_range_choice'],
            'last_7_days',
            "Assert that the macro date value choice last_7_days was preserved"
        );
    }
    ...
    public function testSaveDateFields()
    {
        ...
        $formatted_data = unserialize(base64_decode($focus->contents));
        $this->assertEquals(
            $formatted_data['date_modified_advanced'],
            '2009-07-03',
            "Assert that value is in db format ('2009-07-03')"
        );
        ...
        $this->assertEquals(
            $_REQUEST['date_modified_advanced'],
            '07/03/2009',
            "Assert that dates in db format were converted back to user's date preference"
        );
