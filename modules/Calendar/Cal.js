...
CAL.get("title-cal-edit").innerHTML=CAL.lbl_edit;ajaxStatus.hideStatus();CAL.get("btn-save").focus();setTimeout(function(){enableQS(false);disableOnUnloadEditView();},500);}else
...
CAL.disable_buttons=function(){CAL.get("btn-save").setAttribute("disabled","disabled");CAL.get("btn-send-invites").setAttribute("disabled","disabled");CAL.get("btn-delete").setAttribute("disabled","disabled");CAL.get("btn-full-form").setAttribute("disabled","disabled");CAL.get("btn-edit-all-recurrences").setAttribute("disabled","disabled");CAL.get("btn-remove-all-recurrences").setAttribute("disabled","disabled");}
...
CAL.get("btn-delete").removeAttribute("disabled");CAL.get("btn-full-form").removeAttribute("disabled");CAL.get("btn-edit-all-recurrences").removeAttribute("disabled");CAL.get("btn-remove-all-recurrences").removeAttribute("disabled");}
...
CAL.dialog_remove=function(){CAL.deleted_id=CAL.get("record").value;CAL.deleted_module=CAL.get("current_module").value;var remove_all_recurrences=CAL.get("edit_all_recurrences").value;var callback={success:function(o){try{res=eval("("+o.responseText+")");}catch(err){alert(CAL.lbl_error_saving);CAL.editDialog.cancel();ajaxStatus.hideStatus();return;}
var e,cell_id;if(e=CAL.get(CAL.deleted_id))
cell_id=e.parentNode.id;if(CAL.view=='shared'){CAL.remove_shared(CAL.deleted_id,remove_all_recurrences);}else{if(e=CAL.get(CAL.deleted_id)){e.parentNode.removeChild(e);CAL.destroy_ui(CAL.deleted_id);}
CAL.basic.remove({record:CAL.deleted_id,user_id:CAL.current_user_id});if(CAL.enable_repeat&&remove_all_recurrences){var nodes=CAL.query("div.act_item[repeat_parent_id='"+CAL.deleted_id+"']");CAL.each(nodes,function(i,v){CAL.basic.remove({record:nodes[i].getAttribute("record"),user_id:CAL.current_user_id});nodes[i].parentNode.removeChild(nodes[i]);CAL.destroy_ui(nodes[i].id);});}}
