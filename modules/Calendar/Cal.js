 ...
 * "Powered by SugarCRM".
 ********************************************************************************/
var CAL={};CAL.dropped=0;CAL.records_openable=true;CAL.moved_from_cell="";CAL.deleted_id="";CAL.deleted_module="";CAL.old_caption="";CAL.disable_creating=false;CAL.record_editable=false;CAL.tp=false;CAL.tp1=false;CAL.shared_users={};CAL.shared_users_count=0;CAL.script_evaled=false;CAL.recordDialog=false;CAL.settingsDialog=false;CAL.scroll_slot=0;CAL.dom=YAHOO.util.Dom;CAL.get=YAHOO.util.Dom.get;CAL.query=YAHOO.util.Selector.query;CAL.align_divs=function(cell_id){if(!cell_id)
...
cnt++;prev_i=i;}}}}
CAL.add_item_to_grid=function(ActRecord){var duration_text=ActRecord.duration_hours+"h";if(ActRecord.duration_minutes>0)
duration_text+=ActRecord.duration_minutes+"m";var startD=new Date((ActRecord.timestamp)*1000);var suffix="";var id_suffix="";if(ActRecord.user_id!=""&&CAL.pview=='shared'){suffix="_"+CAL.shared_users[ActRecord.user_id];id_suffix='____'+CAL.shared_users[ActRecord.user_id];}
var e=CAL.get(ActRecord.record+id_suffix);if(e)
e.parentNode.removeChild(e);var start_text=CAL.get_header_text(ActRecord.type,ActRecord.time_start,ActRecord.status,ActRecord.record);var time_cell=ActRecord.timestamp-ActRecord.timestamp%(CAL.t_step*60);var duration_coef;if(ActRecord.module_name=='Tasks'){duration_coef=1;duration_text=" ";}else{if((ActRecord.duration_minutes<CAL.t_step)&&(ActRecord.duration_hours==0))
duration_coef=1;else
duration_coef=(parseInt(ActRecord.duration_hours)*60+parseInt(ActRecord.duration_minutes))/ CAL.t_step;}
var item_text="";if(CAL.item_text&&(typeof ActRecord[CAL.item_text]!="undefined"))
item_text=ActRecord[CAL.item_text];var contain_style="";if(duration_coef<1.75)
contain_style="style='display: none;'";var elm_id=ActRecord.record+id_suffix;var el=document.createElement("div");el.innerHTML="<div class='head'><div class='adicon' onmouseover='return CAL.show_additional_details("+'"'+ActRecord.record+id_suffix+'"'+");' onmouseout='return nd(400);' >&nbsp;&nbsp;</div><div>"+start_text+"</div>"+""+"</div><div class='contain' "+contain_style+">"+item_text+"</div>";el.className="act_item"+" "+ActRecord.type+"_item";el.setAttribute("id",elm_id);el.setAttribute("module_name",ActRecord.module_name);el.setAttribute("record",ActRecord.record);el.setAttribute("dur",duration_text);el.setAttribute("subj",ActRecord.record_name);el.setAttribute("date_start",ActRecord.date_start);el.setAttribute("desc",ActRecord.description);el.setAttribute("parent_name",ActRecord.parent_name);el.setAttribute("parent_type",ActRecord.parent_type);el.setAttribute("parent_id",ActRecord.parent_id);el.setAttribute("status",ActRecord.status);el.setAttribute("detailview",ActRecord.detailview);el.setAttribute("editview",ActRecord.editview);el.setAttribute("duration_coef",duration_coef);el.style.backgroundColor=CAL.activity_colors[ActRecord.module_name]['body'];el.style.borderColor=CAL.activity_colors[ActRecord.module_name]['border'];el.style.height=parseInt(15*duration_coef-1)+"px";YAHOO.util.Event.on(el,"click",function(){if(this.getAttribute('detailview')=="1")
CAL.FormLoad(this.getAttribute('module_name'),this.getAttribute('record'),false);});YAHOO.util.Event.on(el,"mouseover",function(){if(!CAL.records_openable)
...
if(CAL.items_draggable&&ActRecord.module_name!="Tasks"&&ActRecord.editview==1){var dd=new YAHOO.util.DDCAL(elm_id,"cal",{isTarget:false,cont:'cal-grid'});dd.onInvalidDrop=function(e){this.el.style.left="-1px";this.el.style.top="-1px";if(CAL.dropped==0){this.el.childNodes[0].innerHTML=CAL.old_caption;}}
...
CAL.cut_record(ActRecord.record+id_suffix);CAL.align_divs("t_"+time_cell+suffix);}}
...
return;var duration_coef=el.getAttribute("duration_coef");var real_celcount=CAL.celcount;if(CAL.pview=='day'||CAL.pview=='week')
...
CAL.init_record_dialog=function(params){CAL.recordDialog=false;var rd=CAL.get("record_dialog");var content=CAL.get("dialog_content");if(CAL.dashlet&&rd){document.getElementById("content").appendChild(rd);}
rd.style.width=params.width+"px";content.style.height=params.height+"px";content.style.overflow="auto";content.style.padding="0";CAL.recordDialog=new YAHOO.widget.Dialog("record_dialog",{fixedcenter:true,draggable:true,visible:false,modal:true,close:true,zIndex:10});var listeners=new YAHOO.util.KeyListener(document,{keys:27},{fn:function(){CAL.recordDialog.cancel();}});CAL.recordDialog.cfg.queueProperty("keylisteners",listeners);CAL.recordDialog.cancelEvent.subscribe(function(e,a,o){CAL.close_record_dialog();});rd.style.display="block";CAL.recordDialog.render();rd.style.overflow="auto";rd.style.overflowX="hidden";rd.style.outline="0 none";rd.style.height="auto";}
CAL.open_record_dialog=function(params){CAL.recordDialog.show();var nodes=CAL.query("#record_tabs li a");CAL.each(nodes,function(i,v){YAHOO.util.Event.on(nodes[i],'click',function(){var nodes_li=CAL.query("#record_tabs li");CAL.each(nodes_li,function(j,v){CAL.dom.removeClass(nodes_li[j],"selected");});if(!CAL.dom.hasClass(this.parentNode,"selected"))
CAL.dom.addClass(this.parentNode,"selected");CAL.select_tab(this.getAttribute("tabname"));});});var nodes_li=CAL.query("#record_tabs li");CAL.each(nodes_li,function(j,v){CAL.dom.removeClass(nodes_li[j],"selected");if(j==0)
...
CAL.close_record_dialog=function(){CAL.clearFields();}
CAL.remove_record_dialog=function(){var rd_c=CAL.get("record_dialog_c");if(rd_c){rd_c.parentNode.removeChild(rd_c);}}
CAL.clearFields=function(){var e;document.getElementById("form_content").innerHTML="";document.forms["CalendarEditView"].elements["current_module"].value="Meetings";CAL.get("radio_call").removeAttribute("disabled");CAL.get("radio_meeting").removeAttribute("disabled");CAL.get("radio_call").checked=false;CAL.get("radio_meeting").checked=true;CAL.get("send_invites").value="";if(e=CAL.get("record"))
e.value="";if(e=CAL.get("list_div_win"))
e.style.display="none";if(e=CAL.get("edit_all_recurrences_btn"))
e.style.display="none";var nodes=CAL.query("#repeat_type option[value='']");CAL.each(nodes,function(i,v){nodes[i].setAttribute("selected","selected");});CAL.GR_update_focus("Meetings","");CAL.select_tab("record_tabs-1");}
CAL.select_tab=function(tid){var nodes=CAL.query("#record_tabs .yui-content");CAL.each(nodes,function(i,v){nodes[i].style.display="none";});var nodes=CAL.query("#record_tabs #"+tid);CAL.each(nodes,function(i,v){nodes[i].style.display="block";});}
CAL.GR_update_user=function(user_id){var callback={success:function(o){res=eval(o.responseText);GLOBAL_REGISTRY.focus.users_arr_hash=undefined;}};var data={"users":user_id};var url="index.php?module=Calendar&action=AjaxGetGRUsers&sugar_body_only=true";YAHOO.util.Connect.asyncRequest('POST',url,callback,CAL.toURI(data));}
...
CAL.get("radio_call").checked=true;if(res.editview==1){CAL.record_editable=true;}else{CAL.record_editable=false;}
CAL.get("radio_call").setAttribute("disabled","disabled");CAL.get("radio_meeting").setAttribute("disabled","disabled");eval(res.gr);SugarWidgetScheduler.update_time();if(CAL.record_editable){CAL.get("btn_save").removeAttribute("disabled");CAL.get("btn_delete").removeAttribute("disabled");CAL.get("btn_apply").removeAttribute("disabled");CAL.get("btn_send_invites").removeAttribute("disabled");}
CAL.get("form_content").style.display="";CAL.get("title-record_dialog").innerHTML=CAL.lbl_edit;ajaxStatus.hideStatus();if(typeof changeParentQS!='undefined')
changeParentQS("parent_name");setTimeout(function(){enableQS(false);disableOnUnloadEditView();},500);}else
alert(CAL.lbl_error_loading);},failure:function(){alert(CAL.lbl_error_loading);}};var url="index.php?module=Calendar&action=AjaxLoadForm&sugar_body_only=true";var data={"current_module":module_name,"record":record};YAHOO.util.Connect.asyncRequest('POST',url,callback,CAL.toURI(data));}
CAL.records_openable=true;}
CAL.removeSharedById=function(record_id){var e;var cell_id;if(e=CAL.get(record_id+'____'+"0"))
...
e.parentNode.removeChild(e);CAL.align_divs(cell_id+'_'+v);});if(e=CAL.get(record_id))
e.parentNode.removeChild(e);CAL.align_divs(cell_id);}
CAL.add_items=function(res){if(CAL.pview!='shared'){CAL.add_item_to_grid(res);}else{CAL.removeSharedById(res.record);record_id=res.record;var timestamp=res.timestamp;CAL.each(res.users,function(i,v){var rec=res;rec.timestamp=timestamp;rec.user_id=v;rec.record=record_id;CAL.add_item_to_grid(rec);CAL.each(rec.arr_rec,function(j,r){rec.record=r.record;rec.timestamp=r.timestamp;CAL.add_item_to_grid(rec);});});}}
CAL.move_activity=function(box_id,slot_id,ex_slot_id){var u,s;if(u=CAL.get(box_id)){if(s=CAL.get(slot_id)){s.appendChild(u);CAL.align_divs(slot_id);CAL.align_divs(ex_slot_id);CAL.cut_record(box_id);var start_text=CAL.get_header_text(CAL.act_types[u.getAttribute('module_name')],s.getAttribute('time'),u.getAttribute('status'),u.getAttribute('record'));u.setAttribute("date_start",s.getAttribute("datetime"));u.childNodes[0].childNodes[1].innerHTML=start_text;}}}
...
return;var e,user_name,user_id,date_start;CAL.get("title-record_dialog").innerHTML=CAL.lbl_loading;document.forms["CalendarEditView"].elements["current_module"].value=mod_name;CAL.current_params.module_name=mod_name;CAL.load_create_form(CAL.current_params);}
CAL.load_create_form=function(params){ajaxStatus.showStatus(SUGAR.language.get('app_strings','LBL_LOADING'));var callback={success:function(o){res=eval("("+o.responseText+")");if(res.success=='yes'){var fc=document.getElementById("form_content");CAL.script_evaled=false;fc.innerHTML='<script type="text/javascript">CAL.script_evaled = true;</script>'+res.html;if(!CAL.script_evaled){SUGAR.util.evalScript(res.html);}
CAL.get("record").value="";CAL.get("current_module").value=res.module_name;var mod_name=res.module_name;if(res.editview==1){CAL.record_editable=true;}else{CAL.record_editable=false;}
CAL.get("title-record_dialog").innerHTML=CAL.lbl_create_new;SugarWidgetScheduler.update_time();setTimeout(function(){enableQS(false);disableOnUnloadEditView();},500);ajaxStatus.hideStatus();}else{alert(CAL.lbl_error_loading);ajaxStatus.hideStatus();}},failure:function(){alert(CAL.lbl_error_loading);ajaxStatus.hideStatus();}};var url="index.php?module=Calendar&action=AjaxLoadForm&sugar_body_only=true";var data={"current_module":params.module_name,"assigned_user_id":params.user_id,"assigned_user_name":params.user_name,"date_start":params.date_start,"duration_hours":1,"duration_minutes":0};YAHOO.util.Connect.asyncRequest('POST',url,callback,CAL.toURI(data));}
...
CAL.dialog_save=function(){ajaxStatus.showStatus(SUGAR.language.get('app_strings','LBL_SAVING'));CAL.get("title-record_dialog").innerHTML=CAL.lbl_saving;CAL.fill_invitees();var callback={success:function(o){res=eval('('+o.responseText+')');if(res.success=='yes'){CAL.add_items(res);CAL.recordDialog.cancel();CAL.update_vcal();ajaxStatus.hideStatus();}else{alert(CAL.lbl_error_saving);ajaxStatus.hideStatus();}},failure:function(){alert(CAL.lbl_error_saving);ajaxStatus.hideStatus();}};var url="index.php?module=Calendar&action=AjaxSave&sugar_body_only=true";YAHOO.util.Connect.setForm(CAL.get("CalendarEditView"));YAHOO.util.Connect.asyncRequest('POST',url,callback,false);}
CAL.dialog_apply=function(){ajaxStatus.showStatus(SUGAR.language.get('app_strings','LBL_SAVING'));CAL.get("title-record_dialog").innerHTML=CAL.lbl_saving;CAL.fill_invitees();var e;if(e=CAL.get("radio_call"))
e.setAttribute("disabled","disabled");if(e=CAL.get("radio_meeting"))
e.setAttribute("disabled","disabled");CAL.get("btn_apply").setAttribute("disabled","disabled");var callback={success:function(o){res=eval('('+o.responseText+')');if(res.success=='yes'){var e;CAL.get("record").value=res.record;CAL.add_items(res);CAL.update_vcal();CAL.get("title-record_dialog").innerHTML=CAL.lbl_edit;if(e=CAL.get("send_invites"))
e.removeAttribute("checked");ajaxStatus.hideStatus();}else{alert(CAL.lbl_error_saving);ajaxStatus.hideStatus();}},failure:function(){alert(CAL.lbl_error_saving);ajaxStatus.hideStatus();}};var url="index.php?module=Calendar&action=AjaxSave&sugar_body_only=true";YAHOO.util.Connect.setForm(CAL.get("CalendarEditView"));YAHOO.util.Connect.asyncRequest('POST',url,callback,false);}
CAL.dialog_remove=function(){CAL.deleted_id=CAL.get("record").value;CAL.deleted_module=CAL.get("current_module").value;var delete_recurring=false;var callback={success:function(o){res=eval('('+o.responseText+')');var e,cell_id;if(e=CAL.get(CAL.deleted_id))
cell_id=e.parentNode.id;if(CAL.pview=='shared')
CAL.removeSharedById(CAL.deleted_id);if(e=CAL.get(CAL.deleted_id))
e.parentNode.removeChild(e);CAL.align_divs(cell_id);},failure:function(){alert(CAL.lbl_error_saving);}};var data={"current_module":CAL.deleted_module,"record":CAL.deleted_id,"delete_recurring":delete_recurring};var url="index.php?module=Calendar&action=AjaxRemove&sugar_body_only=true";YAHOO.util.Connect.asyncRequest('POST',url,callback,CAL.toURI(data));CAL.recordDialog.cancel();}
CAL.show_additional_details=function(d_id){var obj=CAL.get(d_id);var record=obj.getAttribute("record");mod=obj.getAttribute("module_name");var atype=CAL.act_types[mod];var subj=obj.getAttribute("subj");var date_start=obj.getAttribute("date_start");var duration=obj.getAttribute("dur");var desc=obj.getAttribute("desc");var detailview=parseInt(obj.getAttribute("detailview"));var editview=parseInt(obj.getAttribute("editview"));var related="";if(obj.getAttribute("parent_id")!='')
...
return"";var date_lbl=CAL.lbl_start_t;var duration_text='<b>'+CAL.lbl_duration+':</b> '+duration+'<br>';if(mod=="Tasks"){date_lbl=CAL.lbl_due_t;duration_text="";}
var caption="<div style='float: left;'>"+CAL.lbl_title+"</div><div style='float: right;'>";if(editview){caption+="<a title=\'"+SUGAR.language.get('app_strings','LBL_EDIT_BUTTON')+"\' href=\'index.php?module="+mod+"&action=EditView&record="+record+"\'><img border=0  src=\'"+CAL.img_edit_inline+"\'></a>";}
if(detailview){caption+="<a title=\'"+SUGAR.language.get('app_strings','LBL_VIEW_BUTTON')+"\' href=\'index.php?module="+mod+"&action=DetailView&record="+record+"\'><img border=0  style=\'margin-left:2px;\' src=\'"+CAL.img_view_inline+"\'></a>";}
caption+="<a title=\'"+SUGAR.language.get('app_strings','LBL_ADDITIONAL_DETAILS_CLOSE_TITLE')+"\' href=\'javascript:return cClick();\' onclick=\'javascript:return cClick();\'><img border=0  style=\'margin-left:2px;margin-right:2px;\' src=\'"+CAL.img_close+"\'></a></div>";var body='<b>'+CAL.lbl_name+':</b> '+subj+' <br><b>'+date_lbl+':</b> '+date_start+'<br>'+duration_text+related+desc;return overlib(body,CAPTION,caption,DELAY,200,STICKY,MOUSEOFF,200,WIDTH,300,CLOSETEXT,'',CLOSETITLE,SUGAR.language.get('app_strings','LBL_ADDITIONAL_DETAILS_CLOSE_TITLE'),CLOSECLICK,FGCLASS,'olFgClass',CGCLASS,'olCgClass',BGCLASS,'olBgClass',TEXTFONTCLASS,'olFontClass',CAPTIONFONTCLASS,'olCapFontClass ecCapFontClass',CLOSEFONTCLASS,'olCloseFontClass');}
CAL.toggleDisplay=function(id){if(document.getElementById(id).style.display=='none'){document.getElementById(id).style.display='inline'
...
CAL.goto_date_call=function(){var date_string=CAL.get("goto_date").value;var date_arr=[];date_arr=date_string.split("/");window.location.href="index.php?module=Calendar&view="+CAL.pview+"&day="+date_arr[1]+"&month="+date_arr[0]+"&year="+date_arr[2];}
...
return object;}
CAL.getStyle=function(oElm,strCssRule){var strValue="";if(document.defaultView&&document.defaultView.getComputedStyle){strValue=document.defaultView.getComputedStyle(oElm,"").getPropertyValue(strCssRule);}
...
CAL.fit_grid=function(){var day_width;var cal_width=document.getElementById("cal-grid").parentNode.offsetWidth;if(CAL.pview=="day")
day_width=parseInt((cal_width-80));else
day_width=parseInt((cal_width-80)/ 7);var nodes=CAL.query("#cal-grid div.day_col");CAL.each(nodes,function(i,v){nodes[i].style.width=day_width+"px";});document.getElementById("cal-grid").style.visibility="";}
