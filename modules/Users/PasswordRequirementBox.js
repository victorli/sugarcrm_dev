...
function password_confirmation() {
	...
	if (new_pwd != ""
			|| confirm_pwd.value != ""
			|| old_pwd != ""
			|| (document.getElementById('page') && document
					.getElementById('page').value == "Change"))
...
function set_password(form, rules) {
	if (form.password_change.value == 'true') {
		...
		if (form.is_admin.value != 1
				&& (form.is_current_admin && form.is_current_admin.value != '1')
				&& (form.is_tenant_admin && form.is_tenant_admin.value != '1')
...
function newrules(minpwdlength, maxpwdlength, customregex) {
	...
	if (document.getElementById('lengths')) {
		...
		if ((length < parseInt(minpwdlength) && parseInt(minpwdlength) > 0)
				|| (length > parseInt(maxpwdlength) && parseInt(maxpwdlength) > 0)) {
			document.getElementById('lengths').className = 'bad';
			good_rules = 1;
		} else {
			document.getElementById('lengths').className = 'good';
		}
	}
	if (document.getElementById('1lowcase')) {
		if (!passwd.match('[abcdefghijklmnopqrstuvwxyz]')) {
			...
			document.getElementById('1lowcase').className = 'good';
		}
	}
	if (document.getElementById('1upcase')) {
		if (!passwd.match('[ABCDEFGHIJKLMNOPQRSTUVWXYZ]')) {
			...
			document.getElementById('1upcase').className = 'good';
		}
	}
	if (document.getElementById('1number')) {
		if (!passwd.match('[0123456789]')) {
			...
			document.getElementById('1number').className = 'good';
		}
	}
	if (document.getElementById('1special')) {
		...
		if (!custom_regex.test(passwd)) {
			...
			document.getElementById('1special').className = 'good';
		}
	}
				...
				for (j = 0; j < document.forms[i].elements.length; j++) {
					...
					if ((field.type == "text" || field.type == "textarea" || field.type == "password")
							...
							&& (field.name == "first_name"
									|| field.name == "name"
									|| field.name == "user_name" || field.name == "document_name")) {
						field.focus();
						if (field.type == "text") {
							field.select();
						}
