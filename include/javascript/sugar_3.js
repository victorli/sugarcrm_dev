...
sugarListView.get_num_selected=function(){if(typeof document.MassUpdate!='undefined'){the_form=document.MassUpdate;for(var wp=0;wp<the_form.elements.length;wp++){if(typeof the_form.elements[wp].name!='undefined'&&the_form.elements[wp].name=='selectCount[]'){return the_form.elements[wp].value;}}}
return 0;}
...
sugarListView.prototype.toggleSelected=function(){var numSelected=sugarListView.get_num_selected();var selectedRecords=document.getElementById("selectedRecordsTop");var selectActions=document.getElementById("selectActions");var selectActionsDisabled=document.getElementById("selectActionsDisabled");if(numSelected>0){selectedRecords.style.display="inline-block";$(".selectActionsDisabled").css("display","none");jQuery('ul[name=selectActions]').each(function(){jQuery(this).css("display","inline-block");});}else{selectedRecords.style.display="none";$(".selectActionsDisabled").css("display","inline-block");jQuery('ul[name=selectActions]').each(function(){jQuery(this).css("display","none");});}}
...
document.MassUpdate.massall.disabled=true;for(var wp=0;wp<inputs_array.length;wp++){if(inputs_array[wp].name=="mass[]"){inputsCount++;if(theForm.select_entire_list.value==1){inputs_array[wp].checked=true;inputs_array[wp].disabled=true;checkedCount++;}
